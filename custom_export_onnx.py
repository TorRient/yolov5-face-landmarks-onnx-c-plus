...
class my_yolov5_model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x[1]
        x[0] = x[0].view(batch, -1, self.no)
        x[1] = x[1].view(batch, -1, self.no)
        x[2] = x[2].view(batch, -1, self.no)
        return torch.cat(x, 1) #Batchx25200x16
        
if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', type=str, default='models/yolov5s.yaml', help='yaml file path')
    parser.add_argument('--weights', type=str, default='yolov5s-face.pt', help='model.pt path')
    ...
    input = torch.zeros(2, 3, opt.imgsize, opt.imgsize).to(device)
    output = net(input)
    ...
    for k, m in net.named_modules():
        ...
        if isinstance(m, Conv):  # assign export-friendly activations
            if isinstance(m.act, nn.Hardswish):
                ...
                m.act = SiLU()
    torch.onnx.export(net, input, f, verbose=False, opset_version=12, input_names=['input'], output_names=['output'],
